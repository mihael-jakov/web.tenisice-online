      ...
      <div class="w-[full] flex pl-8 max-[480px]:w-[100%] max-[480px]:pl-0 overflow-hidden">
        <ul class="tenisice_visibility" style="display: flex; flex-wrap: wrap; ">
            <li 
                ...
                v-for="naruciti_yeezy in dostupno_yeezy.products"
                :key="naruciti_yeezy.id"
                @click="gotoproductpage(naruciti_yeezy.id)"
            >
              ...
              <div class="flex text-start justify-start items-start flex-col py-8  max-[480px]:px-2 cursor-pointer">
                <div class="w-full max-[400px]:w-auto images">
                    <img :src="naruciti_yeezy.image2" alt="" loading="lazy" class="image1 inline w-[350px] h-[400px] object-cover max-[1105px]:w-[90%] max-[1105px]:hidden">
                  <img :src="naruciti_yeezy.image" alt="" loading="lazy" class="image2 inline w-[350px] h-[400px] object-cover max-[1105px]:w-[90%] max-[1105px]:h-[400px] max-[480px]:h-[220px] max-[480px]:w-[180px] max-[380px]:h-[250px] max-[380px]:w-[full]">
                </div>
                <p class="text-[1.2rem] my-2 leading-[1.5] font-semibold text-[white] mt-4 w-[80%] max-[480px]:w-full">{{ naruciti_yeezy.name }}</p>
                ...
                <div class="flex gap-4 items-center my-2 max-[380px]:mb-4 text-start ">
                  <p class="line-through text-[white] text-[13px]">{{ naruciti_yeezy.price_before }}€</p>
                  <p class="text-[1.4rem] text-[white]">{{ naruciti_yeezy.price }}€</p>
                </div>
              </div>
            </li>
...
<script setup>
    ...
    import { naruciti_yeezy } from '../../stores/naruciti_yeezy'
    import { onMounted } from 'vue';
    import { useRouter } from "vue-router"

    const dostupno_yeezy = naruciti_yeezy();
    ...
    onMounted(() =>{
        dostupno_yeezy.fetchProductsFromDB();
